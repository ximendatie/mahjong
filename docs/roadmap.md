...
| 7 | 1.x | 生态扩展与自动更新 | Sparkle 决策/集成、更多 Provider、Provider 模板、反馈闭环。 |
...
第七阶段承接 1.0 之后的稳定基础，重点不是把 mahjong 变成云服务或任务编排平台，而是让它更容易更新、更容易扩展、更容易从真实用户反馈里成长。

| 优先级 | 工作项 | 原因 |
| --- | --- | --- |
| Done | 程序坞入口 | 普通 macOS 用户能在 Dock 里看到、切换和退出 mahjong；应用图标复用红中麻将牌。 |
| Done | Dock 图标开关 | 熟悉后可隐藏 Dock 入口，只保留菜单栏和桌宠常驻。 |
...
| P1 | 本地健康状态 | 展示刷新耗时、最近成功刷新时间、失败 Provider 数量，但不上传遥测。 |
| P2 | 低打扰通知偏好 | 只在用户明确开启时提醒任务完成或中断。 |
| P2 | 反馈闭环 | 更新 launch checklist、issue 标签和 release notes 模板。 |

验收标准：

- App 在 Dock 中有入口，并使用红中麻将牌图标。
- 自动更新有明确决策：已集成 Sparkle，或明确延期并有手动更新说明。
- 新 Provider 可以按模板创建，并包含 fixture 测试和隐私文档更新。
- 至少完成一个新 Provider 的初始支持或运行态检测。
- 反馈入口能收集 Provider 名称、数据来源、权限需求、样例 fixture 和隐私风险。
- 1.x 仍保持本地优先、只读观察，不引入云同步、登录系统或任务控制。
...
| 9 | 1.0.0 | 稳定产品承诺 |
| 10 | 1.1.0 | 程序坞入口、自动更新决策、Provider 请求模板 |
| 11 | 1.2.0 | Provider scaffold、新 Provider 初始支持、本地健康状态 |
...
| Done | Add Dock entry with red Mahjong tile icon |
| Done | Decide Sparkle auto-update strategy for 1.x |
| Done | Add Provider request issue template |
| Done | Add Provider scaffold/template |
